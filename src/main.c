...
const char PIPE_LINE[] = "videotestsrc pattern=ball ! video/x-raw,width=1920,height=1080,framerate=30/1 ! videoconvert ! queue ! x264enc bitrate=600000 speed-preset=ultrafast tune=zerolatency key-int-max=15 ! video/x-h264,profile=baseline ! rtph264pay name=rtp config-interval=500 ssrc=1 ! appsink name=peer-connection-sink";
